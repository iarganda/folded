...
Once Fiji and the two auxiliary plugins are installed, the file ‘folded_.bsh’ can be downloaded from the [Github repository](https://github.com/iarganda/folded). Next, copy the file to the directory \<Fiji root\>/plugins/Scripts/Plugins/ and restart Fiji. The script then appears as a new command under the menu [Plugins] (in the last position of the menu). 
